...
let usernameArgs = {
  where: { username: 'iotplatform', connected: true }
}
...
test.beforeEach(async () => {
  ...
  AgentStub.findAll.withArgs(usernameArgs).returns(Promise.resolve(agentFixtures.iotplatform))
...
test.serial('Agent#findByUsername', async t => {
  let agents = await db.Agent.findByUsername('iotplatform')

  t.true(AgentStub.findAll.called, 'findAll should be called on model')
  t.true(AgentStub.findAll.calledOnce, 'findAll should be called once')
  t.true(AgentStub.findAll.calledWith(usernameArgs), 'findAll should be called with username args')

  t.is(agents.length, agentFixtures.iotplatform.length, 'agents should be the same amount')
  t.deepEqual(agents, agentFixtures.iotplatform, 'agents should be the same')
})
